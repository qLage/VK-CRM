...
router.post('/', authenticateToken, [
    ...
    try {
        ...
        await logAudit(req, 'CREATE', 'setting', key, { name: key });
...
router.delete('/:key', authenticateToken, requireAccessLevel(90), async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const existingResult = await query('SELECT key FROM system_settings WHERE key = $1', [key]);
        const existing = existingResult.rows[0];
        await query('DELETE FROM system_settings WHERE key = $1', [key]);
        await logAudit(req, 'DELETE', 'setting', key, { name: existing?.key || key });
