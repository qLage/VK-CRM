    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
         ...
         VALUES ($1, $2, $3, $4)`,
            ...
            await logAudit(req, 'CREATE', 'template', template.id, { name: template.title || '' });
...
router.delete('/:id',
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            const { id } = req.params;
            const existingResult = await query('SELECT title FROM report_templates WHERE id = $1', [id]);
            const existing = existingResult.rows[0];
            await query('UPDATE report_templates SET is_active = 0 WHERE id = $1', [id]);
            await logAudit(req, 'DELETE', 'template', id, { name: existing?.title || '' });
