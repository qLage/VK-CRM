...
router.post('/', authenticateToken, requireAccessLevel(90), [
  ...
  try {
    ...
    await logAudit(req, 'CREATE', 'commission_rule', rule.id, { name: rule.document_type || rule.property_type || '' });
...
router.put('/:id', authenticateToken, requireAccessLevel(90), [
  ...
  try {
    ...
    await logAudit(req, 'UPDATE', 'commission_rule', rule.id, { name: rule.document_type || rule.property_type || '', ...req.body });
    res.json(rule);
  } catch (error) {
...
router.delete('/:id', authenticateToken, requireAccessLevel(90), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await logAudit(req, 'DELETE', 'commission_rule', rule.id, { name: rule.document_type || rule.property_type || '' });
