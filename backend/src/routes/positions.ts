...
router.post('/', authenticateToken, requirePermission('can_manage_users'), invalidateCacheMiddleware(), [
    ...
    await logAudit(req, 'CREATE', 'position', id, { name: position?.name || '' });
...
router.patch('/:id', authenticateToken, requirePermission('can_manage_users'), invalidateCacheMiddleware(), [
    ...
    if (name !== undefined && !existing.is_system) changed.name = name;
    if (description !== undefined) changed.description = description;
    if (base_salary !== undefined) changed.base_salary = base_salary;
    if (commission_percent !== undefined) changed.commission_percent = commission_percent;
    if (default_personal_kpi_min !== undefined) changed.default_personal_kpi_min = default_personal_kpi_min;
    if (default_personal_kpi_max !== undefined) changed.default_personal_kpi_max = default_personal_kpi_max;
    if (default_management_kpi_min !== undefined) changed.default_management_kpi_min = default_management_kpi_min;
    if (default_management_kpi_max !== undefined) changed.default_management_kpi_max = default_management_kpi_max;
    if (management_base_salary !== undefined) changed.management_base_salary = management_base_salary;
    if (participates_in_rating !== undefined) changed.participates_in_rating = participates_in_rating;
    if (is_salary_enabled !== undefined) changed.is_salary_enabled = is_salary_enabled;
    if (is_kpi_enabled !== undefined) changed.is_kpi_enabled = is_kpi_enabled;
    if (is_new_building !== undefined) changed.is_new_building = is_new_building;

    await logAudit(req, 'UPDATE', 'position', id, { name: position?.name || '', ...changed }, existing);
    res.json(position);
}));
...
router.delete('/:id', authenticateToken, requirePermission('can_manage_users'), invalidateCacheMiddleware(), asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const existingResult = await query('SELECT id, is_system, name FROM positions WHERE id = $1', [id]);
    if (existingResult.rows.length === 0) {
        throw new AppError('Position not found', 404, 'POSITION_NOT_FOUND');
    }
    ...
    await logAudit(req, 'DELETE', 'position', id, { name: existingResult.rows[0]?.name || '' });
    res.status(204).send();
}));
