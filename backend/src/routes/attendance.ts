...
router.get('/', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    ...
    `;
        ...
        } else if (user!.role === 'realtor' || user!.role === 'mortgage_broker') {
            conditions.push(`a.user_id = $${paramIndex}`);
            params.push(user!.id);
            paramIndex++;
        }
...
router.post('/',
    ...
    async (req: Request, res: Response): Promise<void> => {
        ...
        WHERE a.id = $1`,
            ...
            await logAudit(req, 'CREATE', 'attendance', result.rows[0].id, { name: result.rows[0].user_name || result.rows[0].date || '' });
...
router.patch('/:id',
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            await logAudit(req, 'UPDATE', 'attendance', id, { name: result.rows[0].user_name || result.rows[0].date || '', ...req.body });
...
router.post('/check-out',
    ...
    async (req: Request, res: Response): Promise<void> => {
        ...
        WHERE a.id = $1`,
            ...
            await logAudit(req, 'UPDATE', 'attendance', record.id, { name: result.rows[0].user_name || result.rows[0].date || '' }, record);
            res.json(result.rows[0]);
        } catch (error: any) {
