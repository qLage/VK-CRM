...
router.post('/', authenticateToken, requireAccessLevel(50), [
    ...
    try {
        ...
        await logAudit(req, 'CREATE', 'team', id, { name: name || '' });
...
router.put('/:id', authenticateToken, requireAccessLevel(50), [
    ...
    try {
        ...
        if (branch_id !== undefined) changes.branch_id = branch_id;
        if (leader_id !== undefined) changes.leader_id = leader_id;
        await logAudit(req, 'UPDATE', 'team', id, { name: name || '', ...changes });
...
router.delete('/:id', authenticateToken, requireAccessLevel(90), async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const existing = await query('SELECT name FROM teams WHERE id = $1 AND company_id = $2', [id, companyId]);
        ...
        await logAudit(req, 'DELETE', 'team', id, { name: existing.rows[0]?.name || '' });
