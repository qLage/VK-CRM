    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
            ...
            await logAudit(req, 'CREATE', 'recurring_expense', row.id, { name: row.name || '' });
...
router.put('/:id',
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            await logAudit(req, 'UPDATE', 'recurring_expense', id, { name: row.name || '', ...req.body });
            res.json(row);
        } catch (error) {
...
router.delete('/:id', authenticateToken, requireAccessLevel(50), async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const existingRes = await query('SELECT name FROM recurring_expenses WHERE id = $1 AND company_id = $2', [id, companyId]);
        ...
        await logAudit(req, 'DELETE', 'recurring_expense', id, { name: existing?.name || '' });
