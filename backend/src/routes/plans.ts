...
router.post('/distribute',
    ...
    async (req: Request, res: Response): Promise<void> => {
        ...
        if (BRANCH_SCOPED_ROLES.includes(userRole) && String(branch_id || '') !== String(userBranchId || '')) {
            res.status(403).json({ error: { message: 'Forbidden: нельзя распределять план на чужой филиал' } });
            return;
        }
          ...
          `, [newId, year, quarter, branch_id, target_revenue, target_deals, target_deposits, target_objects, target_newbuildings, target_mortgage, target_attendance, target_calls, target_meetings, target_showings, req.user!.id]);
                ...
                const allRealtorsResult = await tx.query(`
                    ...
                    WHERE pr.is_active = 1
                      ...
                      AND (ur.role IS NULL OR ur.role NOT IN ('director', 'commercial', 'admin', 'head_sales'))
                      AND COALESCE(pos.name, '') !~* '(директор|админ)'
                      AND COALESCE(pos.access_level, 0) < 90
        ...
        `;
            ...
            await logAudit(req, 'CREATE', 'plan', savedPlan.rows[0].id, { name: `Plan Q${quarter} ${year}` });
            res.json({
...
router.get('/', authenticateToken, requireAccessLevel(50), async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        let paramIndex = 1;
            ...
            const effectiveBranchId = BRANCH_SCOPED_ROLES.includes(userRole)
                ? (userBranchId || undefined)
                : requestedBranchId;
            if (effectiveBranchId) {
                sql += ` WHERE period_year = $${paramIndex} AND period_quarter = $${paramIndex + 1} AND branch_id = $${paramIndex + 2}`;
                params.push(Number(year), Number(quarter), effectiveBranchId);
                paramIndex += 3;
            } else {
...
router.get('/my-plan', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        let userId = (['admin', 'director', 'head_sales', 'sales_manager', 'commercial'].includes(req.user!.role) && requestedUserId)
            ? req.query.userId as string
            : req.user!.id;
        if (BRANCH_SCOPED_ROLES.includes(userRole) && requestedUserId && requestedUserId !== req.user!.id) {
            const targetUserRes = await query('SELECT branch_id FROM profiles WHERE id = $1', [requestedUserId]);
            const targetBranchId = targetUserRes.rows[0]?.branch_id;
            if (String(targetBranchId || '') !== String(userBranchId || '')) {
                res.status(403).json({ error: { message: 'Forbidden: чужой филиал' } });
                return;
            }
            userId = requestedUserId;
        }

        const targetProfileForPlan = await query(
            `SELECT COALESCE(pos.name, '') as position_name, COALESCE(pos.access_level, 0) as access_level
             FROM profiles p
             LEFT JOIN positions pos ON p.position_id = pos.id
             WHERE p.id = $1`,
            [userId]
        );
        const targetPositionName = String(targetProfileForPlan.rows[0]?.position_name || '').toLowerCase();
        const targetAccessLevel = Number(targetProfileForPlan.rows[0]?.access_level || 0);
        const hasNoPersonalPlan = targetAccessLevel >= 90 || /(директор|админ|administrator|admin)/i.test(targetPositionName);
        if (hasNoPersonalPlan) {
            res.json({
                target_revenue: 0,
                target_deals: 0,
                target_deposits: 0,
...
router.get('/users-plans', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        let sql = `SELECT up.user_id,
                          ...
                          SUM(up.target_attendance) as target_attendance
                   FROM user_plans up
        ...
        if (month) {
            whereClauses.push('up.period_month = $1');
            params.push(month);
            ...
            whereClauses.push('up.period_month IN ($1, $2, $3)');
            params.push(...months);
        } else if (year) {
            whereClauses.push('up.period_month LIKE $1');
            params.push(`${year}-%`);
        } else {
        ...
        if (BRANCH_SCOPED_ROLES.includes(userRole)) {
            ...
            params.push(userBranchId || null);
        }
        whereClauses.push(`COALESCE(pos.access_level, 0) < 90`);
        whereClauses.push(`COALESCE(pos.name, '') !~* '(директор|админ|administrator|admin)'`);
        whereClauses.push(`(ur.role IS NULL OR ur.role NOT IN ('director','admin'))`);
...
router.get('/employee-allocations', authenticateToken, requireAccessLevel(50), async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const requestedBranchId = branch_id && branch_id !== 'all' ? String(branch_id) : undefined;
        const effectiveBranchId = BRANCH_SCOPED_ROLES.includes(userRole)
            ? (userBranchId || undefined)
            : requestedBranchId;
        if (effectiveBranchId) {
        ...
        const sql = `
            ...
            WHERE up.period_month IN ($1, $2, $3)
              AND p.is_active = 1
              AND COALESCE(pos.participates_in_rating, 1) = 1
              AND (ur.role IS NULL OR ur.role NOT IN ('director', 'commercial', 'admin'))
              AND COALESCE(pos.access_level, 0) < 90
              AND COALESCE(pos.name, '') !~* '(директор|админ|administrator|admin)'${branchFilter}
...
router.put('/employee/:userId',
    ...
    async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            ...
            const userBranchId = req.user!.branch_id;
            if (BRANCH_SCOPED_ROLES.includes(userRole)) {
                const targetProfileRes = await query('SELECT branch_id FROM profiles WHERE id = $1', [userId]);
                const targetBranchId = targetProfileRes.rows[0]?.branch_id;
                if (String(targetBranchId || '') !== String(userBranchId || '')) {
                    res.status(403).json({ error: { message: 'Forbidden: нельзя редактировать план сотрудника из другого филиала' } });
                    return;
                }
            }
            const months = [
            ...
            await logAudit(req, 'UPDATE', 'plan', null, { name: 'Employee plan update', userId, year, quarter });
