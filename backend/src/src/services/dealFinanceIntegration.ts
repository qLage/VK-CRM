...
class DealFinanceIntegration {
  ...
  static async updatePlanCompletion(employeeId: string, year: number, month: number): Promise<void> {
    ...
    const planResult = await query(`
      SELECT target_revenue FROM user_plans
      WHERE user_id = $1 AND period_year = $2 AND period_month = $3
    `, [employeeId, year, month]);

    if (!planResult.rows?.length) return;

    const targetRevenue = parseFloat(String(planResult.rows[0]?.target_revenue || 0));

    // Get actual revenue using IDs
    const actualResult = await query(`
      SELECT
        SUM(CASE WHEN agent_id = $1 OR rop_id = $1 THEN commission_total_fact ELSE 0 END) as actual_revenue
      FROM deal_table_rows
      WHERE year = $2 AND month = $3 AND status IN ('approved', 'active')
    `, [employeeId, year, month]);

    const actualRevenue = parseFloat(String(actualResult.rows[0]?.actual_revenue || 0));
    const completion = targetRevenue > 0 ? (actualRevenue / targetRevenue) * 100 : 0;
    ...
    await query(`
      ...
      SET
        ...
        updated_at = $3
      WHERE user_id = $4 AND period_year = $5 AND period_month = $6
    `, [
      actualRevenue,
  ...
  static async deleteDealIntegration(dealId: string): Promise<{ success: boolean }> {
    try {
      ...
      console.error('Error deleting deal integration:', error);
      throw error;
    }
  }
}
