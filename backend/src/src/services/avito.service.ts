...
/** @see backend/src/services/avito.service.ts — same logic */
...
function propertyToAdXml(prop: any, photoUrls: string[]): string {
    ...
    if (Category === 'Квартиры') {
        if (prop.rooms) lines.push(`        <Rooms>${escapeXml(String(prop.rooms))}</Rooms>`);
        if (prop.floor) lines.push(`        <Floor>${prop.floor}</Floor>`);
        if (prop.floors_total) lines.push(`        <Floors>${prop.floors_total}</Floors>`);
        ...
        // RoomType — обязательный для 2+ комнат
        if (prop.rooms && Number(prop.rooms) >= 2) {
        ...
        if (isApartmentRent) {
            ...
            // Avito rent feed: explicit yes/no flags for children and pets.
        ...
        // Square
        if (prop.area_total) lines.push(`        <Square>${prop.area_total}</Square>`);

        // BalconyOrLoggiaMulti
        if (prop.balcony && balconyMap[prop.balcony] && prop.balcony !== 'none') {
            lines.push(`        <BalconyOrLoggiaMulti><Option>${escapeXml(balconyMap[prop.balcony])}</Option></BalconyOrLoggiaMulti>`);
        }

        // BathroomMulti
        if (prop.bathroom && bathroomMap[prop.bathroom]) {
            lines.push(`        <BathroomMulti><Option>${escapeXml(bathroomMap[prop.bathroom])}</Option></BathroomMulti>`);
        }

        // SaleOptions
        if (!isApartmentRent) {
            if (prop.sale_options) {
                const opts = String(prop.sale_options).split(',').map(s => s.trim()).filter(Boolean);
                if (opts.length > 0) {
                    lines.push(`        <SaleOptions>${opts.map(o => `<Option>${escapeXml(o)}</Option>`).join('')}</SaleOptions>`);
                }
            } else {
        ...
        if (elev === 'both') {
            lines.push(`        <PassengerElevator>1</PassengerElevator>`);
            lines.push(`        <FreightElevator>1</FreightElevator>`);
        } else if (elev === 'passenger') {
            lines.push(`        <PassengerElevator>1</PassengerElevator>`);
            lines.push(`        <FreightElevator>Нет</FreightElevator>`);
        } else if (elev === 'freight') {
            lines.push(`        <PassengerElevator>Нет</PassengerElevator>`);
            lines.push(`        <FreightElevator>1</FreightElevator>`);
        } else {
            lines.push(`        <PassengerElevator>Нет</PassengerElevator>`);
            lines.push(`        <FreightElevator>Нет</FreightElevator>`);
    ...
    if (Category === 'Дома, дачи, коттеджи') {
        lines.push(`        <ObjectType>Дом</ObjectType>`);
        if (prop.area_total) lines.push(`        <Square>${prop.area_total}</Square>`);
        if (prop.land_area) lines.push(`        <LandArea>${prop.land_area}</LandArea>`);
        if (prop.floors_total) lines.push(`        <Floors>${prop.floors_total}</Floors>`);
        if (prop.rooms) lines.push(`        <Rooms>${prop.rooms}</Rooms>`);
        // WallsType from walls_type or house_type
        ...
        if (wt && wallsTypeMap[wt]) {
            lines.push(`        <WallsType>${escapeXml(wallsTypeMap[wt])}</WallsType>`);
        }
        const houseRenovation = normalizeRenovation(prop.renovation);
        if (houseRenovation) {
            lines.push(`        <Renovation>${escapeXml(houseRenovation)}</Renovation>`);
        }
        if (prop.bathroom && bathroomMap[prop.bathroom]) {
            lines.push(`        <BathroomMulti><Option>${escapeXml(bathroomMap[prop.bathroom])}</Option></BathroomMulti>`);
        }
        // Heating
        const yesNoMap = (v: string | null | undefined) => v === 'yes' ? 'Есть' : v === 'no' ? 'Нет' : null;
        const heatingVal = yesNoMap(prop.heating);
        if (heatingVal) lines.push(`        <Heating>${heatingVal}</Heating>`);
        const wsVal = yesNoMap(prop.water_supply);
        if (wsVal) lines.push(`        <WaterSupply>${wsVal}</WaterSupply>`);
        const sewVal = yesNoMap(prop.sewerage);
        if (sewVal) lines.push(`        <Sewerage>${sewVal}</Sewerage>`);
        const gasVal = yesNoMap(prop.gas_supply);
        if (gasVal) lines.push(`        <GasSupply>${gasVal}</GasSupply>`);
        // LandStatus
        ...
        if (prop.land_status && landStatusMap[prop.land_status]) {
            lines.push(`        <LandStatus>${escapeXml(landStatusMap[prop.land_status])}</LandStatus>`);
        } else if (prop.land_status) {
            lines.push(`        <LandStatus>${escapeXml(prop.land_status)}</LandStatus>`);
        }
    ...
    lines.push(`        <ListingFee>Single</ListingFee>`);
