...
// Feature flags
const USE_NEW_KPI_SERVICE = process.env.USE_NEW_KPI_SERVICE !== 'false';
...
class KpiService {
    ...
    async calculateRealtorKPI(userId, startDate, endDate, period = 'month', role = 'realtor') {
        ...
        let planDeposits = 0, planObjects = 0, planRevenue = 0;
        ...
        if (isQuarter) {
            ...
            const mPlanSql = pool
                ? `SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE user_id = $1 AND period_month IN ($2, $3, $4)`
                : `SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE user_id = ? AND period_month IN (?, ?, ?)`;
            ...
            const mPlanSql = pool ? 'SELECT target_deposits, target_objects, target_revenue FROM user_plans WHERE user_id = $1 AND period_month = $2' : 'SELECT target_deposits, target_objects, target_revenue FROM user_plans WHERE user_id = ? AND period_month = ?';
        ...
        console.log('[calculateRealtorKPI] Plan targets:', { planDeposits, planObjects, planRevenue });
        ...
        // thresholds in KPI_TIERS are already quarterly/fixed values
        const factor = 1;
        const KPI_TIERS = [
        ...
        if (totalRevenue >= 1550000 * factor) currentPercent = 60;
        else if (totalRevenue >= 1200000 * factor) currentPercent = 55;
        else if (totalRevenue >= 900000 * factor) currentPercent = 50;
        else if (totalRevenue >= 700000 * factor) currentPercent = 45;

        // Find current tier index for threshold info
    ...
    async calculateTeamKPI(leaderId, startDate, endDate, period = 'month') {
        ...
        const revenueSql = pool
            ? `SELECT
               ...
               WHERE (dtr.team_id::TEXT = $1 OR dtr.mop_name = $2 OR (dtr.mop_name IS NULL AND dtr.agent_name IN (SELECT full_name FROM profiles WHERE team_id = $3))) 
                 AND ${yearMonthFilter}
                 AND dtr.status IN ('approved', 'active', 'pending', 'new', 'finished')`
            : `SELECT
                COALESCE(SUM(dtr.mop_revenue), 0) as mop_revenue,
                COALESCE(SUM(dtr.commission_total_fact), 0) as total_revenue,
                COALESCE(SUM(dtr.rop_payout), 0) as rop_payout,
        ...
        if (isQuarter) {
            ...
            const qStr = pool
                ? `SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month IN ($1, $2, $3) AND user_id IN (SELECT id FROM profiles WHERE team_id = $4::TEXT)`
                : `SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month IN (?, ?, ?) AND user_id IN (SELECT id FROM profiles WHERE team_id = ?)`;
            ...
            const month = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
            const qStr = pool
                ? 'SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month = $1 AND user_id IN (SELECT id FROM profiles WHERE team_id = $2::TEXT)'
                : 'SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month = ? AND user_id IN (SELECT id FROM profiles WHERE team_id = ?)';
    ...
    async calculateBranchKPI(userId, startDate, endDate, period = 'month', explicitBranchId = null) {
        ...
        if (isQuarter) {
            ...
            if (isGlobal) {
                planQueryStr = pool
                    ? `SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month IN ($1, $2, $3)`
                    : `SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month IN (?, ?, ?)`;
                planParams = [m1, m2, m3];
            } else {
                planQueryStr = pool
                    ? `SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month IN ($1, $2, $3) AND user_id IN (SELECT id FROM profiles WHERE branch_id = $4::TEXT)`
                    : `SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month IN (?, ?, ?) AND user_id IN (SELECT id FROM profiles WHERE branch_id = ?)`;
            ...
            if (isGlobal) {
                planQueryStr = pool
                    ? 'SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month = $1'
                    : 'SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month = ?';
                planParams = [month];
            } else {
                planQueryStr = pool
                    ? 'SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month = $1 AND user_id IN (SELECT id FROM profiles WHERE branch_id = $2)'
                    : 'SELECT SUM(target_deposits) as deposits, SUM(target_objects) as objects, SUM(target_revenue) as revenue FROM user_plans WHERE period_month = ? AND user_id IN (SELECT id FROM profiles WHERE branch_id = ?)';
        ...
        // This ensures directors/commercials see their personal KPI rate and tier progress
        let currentPercent = 40;
        ...
        try {
            ...
            // Calculate tier based on personal revenue (use totalRevenue from branch calculation)
            const isQuarterly = period === 'quarter';
            const KPI_TIERS = isQuarterly
                ? [
                    { percent: 40, threshold: 0 },
                    { percent: 45, threshold: 2100000 },
                    { percent: 50, threshold: 2700000 },
                    { percent: 55, threshold: 3600000 },
                    { percent: 60, threshold: 4650000 },
                  ]
                : [
                    { percent: 40, threshold: 0 },
                    { percent: 45, threshold: 700000 },
            ...
            let currentTierIndex = 0;
            for (let i = KPI_TIERS.length - 1; i >= 0; i--) {
                if (totalRevenue >= KPI_TIERS[i].threshold) {
                    currentTierIndex = i;
                    break;
                }
            }
            ...
            currentThreshold = KPI_TIERS[currentTierIndex].threshold;

            // Get next threshold
            if (currentTierIndex < KPI_TIERS.length - 1) {
                nextThreshold = KPI_TIERS[currentTierIndex + 1].threshold;
            }
    ...
    async getLeaderboard(startDate, endDate, branchId = null, teamId = null) {
        ...
        let profiles = [];
        let queryStr = `
            SELECT p.id, p.full_name, p.avatar_url, r.role, p.team_id, p.branch_id, t.name as team_name,
                ...
                if (pool && userIds.length > 0) {
                    const manualRev = await query(`
                        SELECT user_id::TEXT as user_id,
                          COALESCE(SUM(
                            CASE
                              WHEN type = 'income' AND category IN ('commission','bonus','deal_commission','other_income') THEN amount::double precision
                              WHEN type = 'expense' AND category IN ('premium','salary') THEN amount::double precision
                              ELSE 0
                            END
                          ), 0)::float AS extra
                        FROM transactions
        ...
        if (cacheService) {
            ...
            await cache.set(cacheKey, result, 120); // Fallback: 2 minutes TTL
        }
        return result;
    }
    async getTrends(userId, role, currentStart, currentEnd, branchId = null) {
        ...
        if (role === 'realtor' || role === 'mortgage_broker') {
            ...
            const buildGrossQuery = (s, e, bId) => {
                ...
                let sql = `SELECT COALESCE(SUM(commission_total_fact), 0) as total FROM deal_table_rows WHERE (year * 100 + month) BETWEEN $1 AND $2`;
    ...
    async getCompanyFinancials(startDate, endDate, branchId = null) {
        ...
        const revQuery = `SELECT COALESCE(SUM(commission_total_fact), 0) as total FROM deal_table_rows WHERE ${yearMonthFilter}${branchFilter}`;
    ...
    async getDashboardStats(userId, role, startDate, endDate, branchId = null) {
        ...
        if (isDirector) {
            ...
            activeQuery = `SELECT COUNT(*) as count FROM deal_table_rows WHERE ${yearMonthFilter}${bFilter}`;
        } else if (isManager) {
            ...
            if (role === 'head_sales') {
                activeQuery = `SELECT COUNT(*) as count FROM deal_table_rows WHERE branch_id::TEXT = ${pool ? '$' + (queryParams.length + 1) : '?'} AND ${yearMonthFilter}`;
                queryParams.push(p?.branch_id);
            } else {
                activeQuery = `SELECT COUNT(*) as count FROM deal_table_rows WHERE team_id::TEXT = ${pool ? '$' + (queryParams.length + 1) : '?'} AND ${yearMonthFilter}`;
            ...
            if (pool) {
                activeQuery = `SELECT COUNT(*) as count FROM deal_table_rows dtr LEFT JOIN profiles pf ON dtr.agent_name = pf.full_name WHERE (pf.id = $${queryParams.length + 1} OR dtr.agent_name = $${queryParams.length + 2} OR (dtr.agent_name LIKE '%' || $${queryParams.length + 3} || '%')) AND ${yearMonthFilter}`;
                queryParams.push(userId, myEntry?.name || null, nameSearch || '');
            } else {
                activeQuery = `SELECT COUNT(*) as count FROM deal_table_rows dtr LEFT JOIN profiles pf ON dtr.agent_name = pf.full_name WHERE (pf.id = ? OR dtr.agent_name = ? OR (dtr.agent_name LIKE '%' || ? || '%')) AND ${yearMonthFilter}`;
    ...
    async calculateDualKPI(userId, role, startDate, endDate, period = 'month', branchId = null) {
        ...
        if (cacheService) {
            const cacheParams = {
                ...
                period,
                branchId: branchId || 'all',
                cacheVersion: 'v11' // Incremented for new unified logic
            };
    ...
    async getRevenueForUser(userId, startDate, endDate, role = 'realtor') {
        ...
        const params = pool 
            ? [userId, names, userId, names, userId, names, userId, userId, userId, userId, userId, ...names, ...names] // Expanded for conditional sum
