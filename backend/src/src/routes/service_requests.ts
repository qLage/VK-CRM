...
router.get('/', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const isManagement = ['admin', 'director'].includes(role);
        const isBranchLevel = ['head_sales', 'sales_manager'].includes(role);
        const limit = parseInt(req.query.limit as string) || 50;
        const cursor = req.query.cursor as string | undefined;
        const teamId = req.query.teamId as string | undefined;

        let sql = `
            SELECT sr.*, p.full_name as author_name, p.avatar_url as author_avatar,
        ...
        if (isManagement) {
            ...
            // Team filter active - show team requests EXCLUDING own requests
...
router.put('/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const isManagement = ['admin', 'director', 'commercial'].includes(role);

        const existing = await query('SELECT * FROM service_requests WHERE id = $1 AND company_id = $2', [id, companyId]);
        ...
        if (!isManagement && sr.user_id !== userId) {
...
router.patch('/:id/status', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!['admin', 'director', 'commercial', 'head_sales', 'sales_manager'].includes(req.user!.role)) {
            res.status(403).json({ error: { message: 'Access denied' } });
            return;
        }

        const srResult = await query('SELECT sr.*, p.full_name as author_name FROM service_requests sr LEFT JOIN profiles p ON sr.user_id = p.id WHERE sr.id = $1 AND sr.company_id = $2', [id, companyId]);
...
router.post('/:id/attachments', authenticateToken, upload.single('file'), async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const reqResult = await query('SELECT id FROM service_requests WHERE id = $1 AND company_id = $2', [id, companyId]);
...
router.delete('/:id/attachments/:attId', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const { role, id: userId, company_id: companyId } = req.user!;
        const isManagement = ['admin', 'director', 'commercial', 'head_sales', 'sales_manager'].includes(role);
        ...
        const reqResult = await query('SELECT user_id FROM service_requests WHERE id = $1 AND company_id = $2', [id, companyId]);
        const requestOwnerId = reqResult.rows[0]?.user_id;

        if (!isManagement && requestOwnerId !== userId) {
            res.status(403).json({ error: { message: 'Access denied' } });
            return;
        }
