...
router.get('/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Get photos
        const photos = await query('SELECT id, property_id, file_url, file_name, file_size, sort_order, created_at, mime_type, (file_data IS NOT NULL) as has_data FROM property_photos WHERE property_id = $1 ORDER BY sort_order', [req.params.id]);
    ...
    try {
        ...
        await query(`
            UPDATE properties SET
                ...
                client_id=COALESCE($50,client_id),
                updated_at=$51 ${statusUpdate}
            WHERE id=$52
        `, [
            category, city, address, lat, lng, price,
            area_total, area_living, area_kitchen, rooms, floor, floors_total, description,
...
const servePhotoData = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Fallback for legacy records where photo is stored by path/key, not as DB binary.
        if (typeof file_url === 'string' && file_url.trim()) {
            ...
            if (s3Service.isS3Enabled && !isLocalUploads) {
                try {
                    const signedUrl = await s3Service.getFileUrl(rawUrl);
                    if (signedUrl) {
                        res.redirect(302, signedUrl);
                        return;
                    }
                } catch (e) {
                    console.warn('S3 fallback failed for photo-data:', rawUrl, (e as any)?.message);
                }
            }
...
router.delete('/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        await query('DELETE FROM properties WHERE id = $1 AND company_id = $2', [req.params.id, companyId]);
        res.json({ success: true });
