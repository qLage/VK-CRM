  ...
  async calculate(userId: string, startDate: string, endDate: string, period: string = 'quarter'): Promise<KpiResult> {
    ...
    if (employee.team_id) {
      if (period === 'quarter') {
        ...
        const planSql = pool
          ? `SELECT SUM(target_revenue) as target_revenue FROM user_plans WHERE user_id IN (SELECT p.id FROM profiles p LEFT JOIN positions pos ON p.position_id = pos.id WHERE p.team_id = $1::TEXT AND COALESCE(pos.access_level, 0) < 90) AND period_month = ANY($2)`
          : `SELECT SUM(target_revenue) as target_revenue FROM user_plans WHERE user_id IN (SELECT p.id FROM profiles p LEFT JOIN positions pos ON p.position_id = pos.id WHERE p.team_id = ? AND COALESCE(pos.access_level, 0) < 90) AND period_month IN (${monthsList.map(() => '?').join(',')})`;
        ...
        const planSql = pool
          ? `SELECT SUM(target_revenue) as target_revenue FROM user_plans WHERE user_id IN (SELECT p.id FROM profiles p LEFT JOIN positions pos ON p.position_id = pos.id WHERE p.team_id = $1::TEXT AND COALESCE(pos.access_level, 0) < 90) AND period_month = $2`
          : `SELECT SUM(target_revenue) as target_revenue FROM user_plans WHERE user_id IN (SELECT p.id FROM profiles p LEFT JOIN positions pos ON p.position_id = pos.id WHERE p.team_id = ? AND COALESCE(pos.access_level, 0) < 90) AND period_month = ?`;
