    ...
    if (userRole === 'director' || userRole === 'commercial') {
      ...
      if (effectiveBranchId && effectiveBranchId !== 'all') {
        if (period === 'quarter') {
          ...
          const planSql = pool
            ? `SELECT SUM(target_revenue) as target_revenue FROM user_plans WHERE user_id IN (SELECT p.id FROM profiles p LEFT JOIN positions pos ON p.position_id = pos.id WHERE p.branch_id = $1::TEXT AND COALESCE(pos.access_level, 0) < 90) AND period_month = ANY($2)`
            : `SELECT SUM(target_revenue) as target_revenue FROM user_plans WHERE user_id IN (SELECT p.id FROM profiles p LEFT JOIN positions pos ON p.position_id = pos.id WHERE p.branch_id = ? AND COALESCE(pos.access_level, 0) < 90) AND period_month IN (${months.map(() => '?').join(',')})`;
          ...
          const planSql = pool
            ? `SELECT SUM(target_revenue) as target_revenue FROM user_plans WHERE user_id IN (SELECT p.id FROM profiles p LEFT JOIN positions pos ON p.position_id = pos.id WHERE p.branch_id = $1::TEXT AND COALESCE(pos.access_level, 0) < 90) AND period_month = $2`
            : `SELECT SUM(target_revenue) as target_revenue FROM user_plans WHERE user_id IN (SELECT p.id FROM profiles p LEFT JOIN positions pos ON p.position_id = pos.id WHERE p.branch_id = ? AND COALESCE(pos.access_level, 0) < 90) AND period_month = ?`;
