...
/**
 * Stream object from bucket to Express response (HTTP 200 + image body).
 * Used for /api/properties/photo-data so Avito and other bots get a direct image URL
 * without following 302 redirects to short-lived signed URLs.
 */
export async function streamObjectToResponse(
    ...
    if (!isS3Enabled || !s3Client) return false;

    let key = String(storageKeyOrUrl || '').trim();
    if (!key) return false;
    ...
    if (key.startsWith('http')) {
        try {
            ...
            const bi = segs.indexOf(bucket);
            if (bi >= 0 && bi < segs.length - 1) {
                key = segs.slice(bi + 1).join('/');
            } else {
                const marker = `${bucket}/`;
    ...
    try {
        ...
        const ct = out.ContentType || options?.contentType || 'image/jpeg';
        ...
        body.on('error', (err) => {
            console.error('[S3] photo stream error:', err?.message || err);
