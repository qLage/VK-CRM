...
// Default tiers - will be overridden by DB values
let DEFAULT_MONTHLY_TIERS: TierThreshold[] = [
...
export function setRealtorTiers(tiers: Array<{ threshold: number; percent: number }>) {
  DEFAULT_MONTHLY_TIERS = tiers.map(t => ({
    threshold: new Decimal(t.threshold),
    percent: new Decimal(t.percent),
  }));
}

export function getRealtorTiers(): TierThreshold[] {
  return [...DEFAULT_MONTHLY_TIERS];
}
 ...
 */
export function calculateRealtorTier(revenue: Decimal, period: 'month' | 'quarter', customTiers?: TierThreshold[]): TierResult {
  const tiers = customTiers || getRealtorTiers();
